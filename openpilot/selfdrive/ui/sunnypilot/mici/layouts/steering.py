...
class SteeringLayoutMici(NavScroller):
  ...
  def __init__(self):
    ...
    self._lc_view = self._lane_change_btn.link_sub_panel([self._lc_timer, self._lc_bsm, self._lc_road_edge,
                                                          self._lc_one_per_signal, self._lc_level])
    ...
    # Mazda TI A/B experiment selector; visible only on the gated platform (see torque_tune.py)
  ...
  def _update_state(self):
    ...
    if alc_val <= AutoLaneChangeMode.OFF and lc_bsm == "off" and road_edge == "off" and lc_level == LEVEL_OFF and one_per_signal == "off":
